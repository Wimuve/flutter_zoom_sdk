...
void main(List<String> args) async {
  ...
  if (isNewFlutter) {
    ...
    for (var package in packageList) {
      if (package["name"] == "zoom_flutter_sdk") {
...
Future<void> checkAndDownloadSDK(String location) async {
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse('https://www.dropbox.com/s/vcwtr7k17uyw2hi/MobileRTC?dl=1'),
        iosSDKFile);
  }
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse(
            'https://www.dropbox.com/s/9mexw9cut2e07k4/commonlib.aar?dl=1'),
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse(
            'https://www.dropbox.com/s/ea5klxep32xqr3u/mobilertc.aar?dl=1'),
        androidRTCLibFile);
  }
}
